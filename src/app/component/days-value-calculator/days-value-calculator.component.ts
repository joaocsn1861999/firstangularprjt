...
export class DaysValueCalculatorComponent implements OnInit {
  ...
  sucessResult(result: number) {
    this.resultField = `O valor referente a ${this.days} dias de acesso do plano de R$${this.plan},00 fica um total de R$${result},00`;
  }
  ...
  daysCalculator() {
    if (parseInt(this.days) > 1 && parseInt(this.plan) >= 70) {
      ...
      this.fieldClenear();
      return (this.resultField = `Quantidade de dias(${this.days}) ou plano(R$${this.plan},00) está invalida! Verifique e tente novamente, por favor.`);
    }
  }

  copyResult() {
    const textareaElement: HTMLTextAreaElement =
      this.resultTextarea.nativeElement;
